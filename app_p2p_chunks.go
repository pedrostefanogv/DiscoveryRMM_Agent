...
// handleArtifactManifest serves GET /p2p/artifact/{name}/manifest.
// It reuses the same token auth as handleArtifact.
func (s *p2pTransferServer) handleArtifactManifest(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}

	// Path: /p2p/artifact/{name}/manifest → strip prefix and suffix.
	rawPath := strings.TrimPrefix(r.URL.Path, "/p2p/artifact/")
	rawPath = strings.TrimSuffix(rawPath, "/manifest")
	name := sanitizeArtifactName(rawPath)
	if name == "" {
	...
	if err != nil {
		http.Error(w, "erro ao construir manifest: "+err.Error(), http.StatusInternalServerError)
		return
	}
