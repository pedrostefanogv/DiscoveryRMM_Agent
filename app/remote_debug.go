...
type remoteDebugStreamConfig struct {
	...
	NatsLegacySubject string `json:"natsLegacySubject"`
	NatsTenantSubject string `json:"natsTenantSubject"`
...
func buildRemoteDebugPublishers(cfg DebugConfig, stream remoteDebugStreamConfig, token string) ([]remoteDebugPublisher, error) {
	...
	subject := strings.TrimSpace(stream.NatsTenantSubject)
	if subject == "" {
		subject = strings.TrimSpace(stream.NatsLegacySubject)
	}
	if subject == "" {
	...
	if p, err := newSignalRRemoteDebugPublisher(cfg, strings.TrimSpace(stream.SignalRHub)); err == nil {
		publishers = append(publishers, p)
	}
...
func newSignalRRemoteDebugPublisher(cfg DebugConfig, hubPath string) (remoteDebugPublisher, error) {
	token := strings.TrimSpace(cfg.AuthToken)
	agentID := strings.TrimSpace(cfg.AgentID)
	...
	return &signalRRemoteDebugPublisher{conn: conn}, nil
}
