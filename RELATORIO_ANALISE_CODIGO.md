...
| **Remote session: input elevado + tela de logon** (homologação 13/09) | ✅ Corrigido — sessão remota movida da UI companion (Medium → UIPI bloqueava) para o worker spawnado pelo serviço (SYSTEM na sessão interativa); token SYSTEM preferencial no spawn |
| **Limpeza de dados no servidor** (homologação 13/09) | ✅ Implementado — menu de manutenção (opção 7) + `CleanupInactiveBuildsAsync` no `RefreshCurrentBuildAsync` e `DataRetentionJob` diário |
| **LOOP self-update** (homologação 13/09) | ✅ **Causa raiz corrigida**: `AgentPackageService.cs` (API) compilava o agente com `-X discovery/internal/buildinfo` (caminho inexistente) → binário sempre 0.0.0/unknown. Corrigido para `discovery/app/core/buildinfo` + Commit injetado no discovery-service. Contrato version+commit + marker + circuit breaker implementados no agente como defesa em profundidade |
| **LOOP self-update** (homologação 13/09) | ✅ Corrigido — **contrato do dono implementado**: comparação version+commit da API contra versão/commit **EFETIVOS** (marker do último install confirmado quando o buildinfo reporta 0.0.0/unknown); pending grava o commit instalado; circuit breaker persistido como segunda camada (3 installs da mesma versão → pausa) |
