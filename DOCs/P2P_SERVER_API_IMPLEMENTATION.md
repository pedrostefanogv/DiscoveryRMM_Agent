...
> Escopo canonico deste documento: implementacao backend do servidor e validacoes normativas.
>
> Para contrato do agente e transporte local/libp2p, usar: DOCs/p2p-api-contract.md
...
> **Nota de compatibilidade (estado atual do agente):** o payload de
> telemetria atualmente enviado pelo Discovery Agent inclui `metrics` e
> `currentSeedPlan`, mas não inclui lista explícita de presença por artifact.
> Em ambientes nesse formato, o backend deve manter `p2p_artifact_presence`
> alimentado por fonte complementar (ex.: inventário de artifacts/cache por
> agente) ou retornar `peerCount`/`peerAgentIds` vazios enquanto a extensão de
> payload por artifact não estiver habilitada no agente.
