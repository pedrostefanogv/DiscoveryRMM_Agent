...
| 2026-09-13 | **Limpeza de dados no servidor**: menu de manutenção (opção 7) + cleanup automático no RefreshCurrentBuildAsync + DataRetentionJob diário. Servidor de homologação: 1 GB de builds + 2.3 GB de journal identificados e limpeza implementada |
...
| **Limpeza de dados no servidor** (homologação 13/09 — 1 GB de builds antigos + 2.3 GB de journal) | Menu de manutenção (opção 7) com limpeza individual ou completa de: builds antigos do agente, journal systemd, cache Go, temp files. **API**: `CleanupInactiveBuildsAsync` integrado ao `RefreshCurrentBuildAsync` (cleanupOldBuilds=true por padrão) e ao `DataRetentionJob` (diário) | `DiscoveryRMM_API/scripts/linux/lib/modes.sh`, `Discovery.Api/Services/Quartz/DataRetentionJob.cs`, `Discovery.Infrastructure/Services/AgentUpdateService.cs` |
| **HTTP 413 no sync de hardware** (homologação 13/09 — 11 falhas, inventário nunca chegou) | nginx do servidor sem `client_max_body_size` (default 1 MB) rejeitava o payload de hardware (`inventoryRaw` embutido, 2-4x maior por escaping). **Fix:** API ganhou `[RequestSizeLimit(64_000_000)]` nos endpoints `me/hardware` POST/PUT; o template nginx do repo **já tinha** `client_max_body_size 100m` — o config implantado no servidor é antigo e precisa ser regenerado (`DISCOVERY_REFRESH_INFRA_CONFIG=1` no update) ou corrigido manualmente | `DiscoveryRMM_API/.../AgentAuthController.cs` |
| **Update da API regenera o nginx por padrão** | O condicional `DISCOVERY_REFRESH_INFRA_CONFIG` foi invertido: **default = regenerar** (antes era opt-in, e o config do servidor ficava para trás). Opt-out: `DISCOVERY_REFRESH_INFRA_CONFIG=0`. O config anterior é preservado em `discovery-rmm.bak` antes de sobrescrever | `DiscoveryRMM_API/scripts/linux/lib/modes.sh` (2 sites), `lib/deploy.sh` (backup) |
| **Warnings do nginx no update** ("conflicting server name localhost/127.0.0.1", "protocol options redefined") | Causa: `discovery-rmm.working` residual de versões antigas do instalador carregado pelo nginx em sites-enabled. **Fix:** `write_site_proxy_config` remove residuais `discovery-rmm.working*` antes do `nginx -t` e avisa (sem remover) sobre configs de terceiros com localhost/127.0.0.1 | `DiscoveryRMM_API/scripts/linux/lib/deploy.sh` |
