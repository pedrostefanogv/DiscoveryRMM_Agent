...
func executeTask(ctx context.Context, packages PackageManager, authorize PackageAuthorizationFunc, task AutomationTask) ExecutionResult {
	...
	case ActionCustomCommand:
		return executeCustomCommand(ctx, task)
	default:
...
func executeScript(ctx context.Context, task AutomationTask) ExecutionResult {
	if task.Script == nil {
	...
	case ScriptPowerShell:
		return executeProcess(ctx, "powershell", []string{"-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-Command", content}, defaultExecutionTimeout)
	case ScriptShell, ScriptBatch, ScriptCustom:
		return executeProcess(ctx, "cmd", []string{"/C", content}, defaultExecutionTimeout)
	case ScriptPython:
		return executeProcess(ctx, "python", []string{"-c", content}, defaultExecutionTimeout)
	default:
		return ExecutionResult{Success: false, ExitCode: 2, ExitCodeSet: true, ErrorMessage: "scriptType nao suportado"}
	}
}

func executeCustomCommand(ctx context.Context, task AutomationTask) ExecutionResult {
	...
	switch commandType {
	case "powershell", "ps":
		return executeProcess(ctx, "powershell", []string{"-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-Command", command}, timeout)
	case "cmd", "shell":
		return executeProcess(ctx, "cmd", []string{"/C", command}, timeout)
	case "exec", "process", "winget":
		return executeProcess(ctx, command, args, timeout)
	default:
		return executeProcess(ctx, "cmd", []string{"/C", command}, timeout)
	}
}
