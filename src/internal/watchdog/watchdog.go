...
type Watchdog struct {
	...
	suspended     map[Component]string
...
func New(cfg Config) *Watchdog {
	...
	return &Watchdog{
		...
		suspended:     make(map[Component]string),
...
// Suspend marks a component as intentionally not monitored for liveness.
func (w *Watchdog) Suspend(component Component, reason string) {
	w.mu.Lock()
	if _, ok := w.heartbeats[component]; !ok {
		w.heartbeats[component] = time.Now()
	}
	if reason = strings.TrimSpace(reason); reason == "" {
		reason = "monitoramento suspenso"
	}
	w.suspended[component] = reason
	w.mu.Unlock()
}

// Resume re-enables health evaluation for a previously suspended component.
func (w *Watchdog) Resume(component Component) {
	w.mu.Lock()
	delete(w.suspended, component)
	w.mu.Unlock()
}
...
func (w *Watchdog) buildHealthCheck(component Component, lastBeat, now time.Time) HealthCheck {
	...
	if reason, ok := w.suspended[component]; ok {
