...
func TestWatchdog_SuspendResume(t *testing.T) {
	wd := New(DefaultConfig())
	wd.Heartbeat(ComponentUI)
	wd.Suspend(ComponentUI, "janela oculta")

	check := wd.GetComponentHealth(ComponentUI)
	if check.Status != StatusUnknown {
		t.Errorf("Expected unknown while suspended, got %s", check.Status)
	}
	if check.Message != "janela oculta" {
		t.Errorf("Expected suspend reason to be preserved, got %q", check.Message)
	}

	wd.Resume(ComponentUI)
	wd.Heartbeat(ComponentUI)

	check = wd.GetComponentHealth(ComponentUI)
	if check.Status != StatusHealthy {
		t.Errorf("Expected healthy after resume, got %s", check.Status)
	}
}
